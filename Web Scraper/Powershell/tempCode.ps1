...
$string = "/ar/arab"
$string2 = "html"
DEL "output\LinksScraped.txt" 2>$NULL
foreach ($link in $site.links.href)
{
	if ($link.length -ge 4)
	{
		if ( $string -eq ($link[0]+$link[1]+$link[2]+$link[3]+$link[4]+$link[5]+$link[6]+$link[7]))
		{
			if ($string2 -eq ($link[$link.length-4] + $link[$link.length-3] +$link[$link.length-2] +$link[$link.length-1]))
			{
				$linksArray += $link
				$link | Out-File "output\LinksScraped.txt" -append -Encoding UTF8
			}
		}
	}
}
